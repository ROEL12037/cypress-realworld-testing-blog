...
describe("Cypress is just JavaScript", () => {
	it("uses _.each() from lodash to make sure the titles from the posts api are displayed correctly on the home page", () => {
		...
		cy.request("GET", "http://localhost:3000/api/posts").then((response) => {
			...
			_.each(sortedPosts(response.body), (val, idx, clctn) => {
				cy.get(`[data-test="post-link-${idx}"]`).contains(val.title);
			});
		});
	});
	...
	it.only("formats the post date correctly on the homepage", () => {
		...
		cy.request("GET", "http://localhost:3000/api/posts").then((response) => {
			...
			_.each(sortedPosts(response.body), (val, idx, clctn) => {
				const date = format(parseISO(val.date), "LLLL d, yyyy");
				cy.get(`[data-test="post-date-${idx}"]`).contains(date);
			});
		});
	});
});
